...
RunMonocle3 <- function(tmp.seu,tmp.assay="integrated",tmp.minimal_branch_len=5){
  ...
  cds <- monocle3::new_cell_data_set(expression_data = mat,
  ...
  ## learn trajectory
  cds <- monocle3::learn_graph(cds,close_loop = F,learn_graph_control = list(minimal_branch_len = tmp.minimal_branch_len,
...
GetVariableGeneInTrajectory <- function(tmp.cds,
  ...
  pr_graph_test_res <- monocle3::graph_test(cds = tmp.cds,
